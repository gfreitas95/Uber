...
class MapViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    @IBAction func sendCoordinates() {
        ...
        if let err = error {
            ...
            let dic = ["date": dateString, "address": self.txtAdress?.text ?? ""]
                
                if var addressDictionary = UserDefaults.standard.value(forKey: "addresses") as? Array<Dictionary<String,String>> { print("added to favorites")
    ...
    func tintPlaceHolder() {
        if let placeholder = txtAdress?.placeholder { txtAdress?.attributedPlaceholder = NSAttributedString(string: placeholder, attributes: [NSAttributedString.Key.foregroundColor: UIColor.white.withAlphaComponent(1)])
        }
    }
}
